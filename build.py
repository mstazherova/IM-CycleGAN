...
def build_model(input_a, input_b, gen_a_sample, gen_b_sample):
    ...
    # Discriminator loss 
    # d_a_loss_real = tf.reduce_mean((d_a - tf.ones_like(d_a)) ** 2)
    ...
    # d_b_loss_real = tf.reduce_mean((d_b - tf.ones_like(d_b)) ** 2)
    ...
    # Generator loss
    g_a_loss = tf.reduce_mean((d_gen_a - tf.ones_like(d_gen_a)) ** 2)
    g_b_loss = tf.reduce_mean((d_gen_b - tf.ones_like(d_gen_b)) ** 2)
